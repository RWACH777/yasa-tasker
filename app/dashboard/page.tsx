...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (!hydratedUser && typeof window !== "undefined") {
  ...
  const loadTasks = async () => {
    ...
    try {
      ...
      else if (error) console.error("Error loading tasks:", error);
    } catch (err) {
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    try {
      const username = hydratedUser?.username || null;
      ...
      <main className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="glass p-4 rounded-2xl border border-white/8">
            ...
            <div className="mt-3">
              <button className="px-3 py-2 bg-blue-600 rounded-lg text-sm" onClick={() => alert("Profile editing coming soon")}>
                Edit Profile
              </button>
            </div>
          </div>
        ...
        <div className="space-y-4">
          {loadingTasks ? (
            ...
            filteredTasks.map((t) => (
              <div key={t.id} className="glass p-4 rounded-xl border border-white/10">
                <div className="flex justify-between items-center">
                  <h3 className="font-semibold">{t.title}</h3>
                  <span className="text-sm text-gray-400">{t.status || "open"}</span>
                </div>
